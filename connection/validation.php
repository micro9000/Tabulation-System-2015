	...
	class validation_form{
		...
		function checkDate($_date){
			...
			if($_date != ''){
				...
				if($err_msg != ''){
					echo "<tr><td colspan ='3' style='color:red; text-align:center;'>".$err_msg."</td></tr>";
					echo "<br/>";
					return false;
				}
			}
		...
		function checkTime($_time){
			...
			if($_time != ''){
				...
				if($err_time != ''){
					echo "<tr><td colspan ='3' style='color:red; text-align:center;'>".$err_time."</td></tr>";
					return false;
				}
			}
			return true;
		...
		function checkTextBox(){
			if(isset($_POST['btn_login'])){
				if(empty($_POST['$_user_name']) && empty($_POST['ad_password'])){
