<?php
	...
	*/
        ...
        public function get_categories($event_id,$round_id){
			...
			*/
            $result = $this -> query("SELECT * FROM category WHERE round_id = $round_id AND event_id = $event_id");
            return $result;
        }
        ...
        public function get_criteria($event_id, $round_id, $category_id){
            ...
            */
            $result = $this -> query("SELECT * FROM criteria WHERE round_id = $round_id AND event_id = $event_id AND category_id = $category_id");
            return $result;
        }
        ...
        public function get_candidate_count($event_id, $gender){
            ...
            if(($this -> connection_check($connection) === true)){
                $this -> _database();//select the database
                $query = "SELECT COUNT(id) AS id FROM candidate_info WHERE event_id = $event_id AND gender = '$gender'";
        ...
        public function get_candidate_count_($event_id){
            ...
            if(($this -> connection_check($connection) === true)){
                $this -> _database();//select the database
                $query = "SELECT COUNT(id) AS id FROM candidate_info WHERE event_id = $event_id";
                $result = mysql_query($query, $connection) or die(mysql_error() . "Line 392");
                $row = mysql_fetch_array($result);
                return $row['id'];
            }
            */
        ...
        public function get_candidate_count_no($event_id){
            ...
            if(($this -> connection_check($connection) === true)){
                $this -> _database();//select the database
                $query = "SELECT COUNT(id) AS id FROM candidate_info WHERE event_id = $event_id";
                $result = mysql_query($query, $connection) or die(mysql_error() . "Line 393");
                $row = mysql_fetch_array($result);
                return $row['id'];
            }
            */
        ...
        public function get_candidate_img($event_id, $candidate_id){
        	...
        	if(($this -> connection_check($connection) === true)){
        		$this -> _database();
        		$query = "SELECT * FROM images WHERE event_id = $event_id AND candidate_id = $candidate_id";
        		$m_query = mysql_query($query, $connection) or die(mysql_error());
        		return $m_query;
        	}
        	*/
